...
module.exports = function makeDataHelpers(knex) {
  ...
  return {
    ...
    createAdminAndEvent: function(firstName, lastName, email, eventName, eventDate, description, newUrl, adminURL, time) {
      return knex('users')
      .returning('id')
      .insert({first_name: firstName, last_name: lastName, email: email})
      .then((user_id) => {
        return knex('events')
        .insert({event_name: eventName, event_date: eventDate, url: newUrl, admin_id: user_id[0], admin_url: adminURL, desc: description, time: time})
      })
    },
